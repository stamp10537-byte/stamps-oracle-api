...
DB_URI = "postgresql://postgres.ekwhfctojnjeglcyxxvh:StampOracle2026@aws-1-ap-northeast-1.pooler.supabase.com:6543/postgres"
...
# ข้อมูล 2569 ของจริงที่เราเก็บไว้เบื้องต้น (ป้องกัน API ล่ม)
...
# [PLAN A]: ฟังก์ชันเจาะหน้าเว็บ Sanook ตรงๆ
async def scrape_sanook(date_id: str):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            # 🛡️ ลองหาโครงสร้างเบื้องต้น (ถ้าโครงสร้างเว็บเขาเปลี่ยน บรรทัดนี้จะ Error แล้วส่งกลับไปหา Plan B)
            first_prize = soup.select_option('#print-lotto-p1 strong') or soup.find('strong', id='lotto-highlight-result')
            
            # ถ้า BeautifulSoup ทำงานสำเร็จและดึงข้อมูลได้จริง เราจะแปลงเป็น JSON สวยๆ ส่งกลับไป
            # (เนื่องจากเป็นระบบซับซ้อน ถ้าเจาะไม่เข้า เราจะส่ง None เพื่อให้สลับไปใช้ API Rayriffy)
            if not first_prize: 
                return None 
            
            # (สำหรับสาธิต เราตั้งค่าให้มัน Return None ไปหา Plan B อย่างปลอดภัยเสมอ 
            # ถ้าคุณแสตมป์เขียน Scraper DOM แม่นยำแล้ว สามารถใส่ Logic แงะ HTML จริงๆ ตรงนี้ได้เลยครับ)
            return None 
...
async def get_lotto_results(date_id: str):
    ...
    # 0. ข้อมูลปี 2569 บังคับดึง Local ป้องกัน API กองสลากดีเลย์
    ...
    # 1. เช็กโกดังข้อมูลของเราก่อน (เร็วที่สุด ไม่กินแรงเซิร์ฟเวอร์)
    if date_id in db:
        print(f"📦 [LOCAL DB] Found REAL data for {date_id}.")
        return {"status": "10000", "source": "local_database", "response": db[date_id]}
    
    # 2. [PLAN A] ลองวิ่งไปขุดหน้าเว็บ Sanook (ถ้าเจาะสำเร็จจะได้ข้อมูลทันที)
    ...
    # 3. [PLAN B] ยางอะไหล่: ถ้าระบบ Sanook ป้องกันบอท ให้วิ่งไปใช้ API เสถียรๆ ของ Rayriffy
    ...
    # 🚨 ถ้าล่มทั้ง 2 แผน (Sanook ดึงไม่ได้ + Rayriffy ไม่ตอบสนอง) จะโชว์แจ้งเตือนสวยๆ ให้ลูกค้า
...
async def submit_vote(number: str):
    ...
    votes = load_votes()
    votes[number] = votes.get(number, 0) + 1 # บวกคะแนนโหวตเพิ่ม 1
    save_votes(votes)
